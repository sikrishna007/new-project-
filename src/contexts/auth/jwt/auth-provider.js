...
export const AuthProvider = (props) => {
  ...
  const initialize = useCallback(async () => {
    try {
      ...
      accessToken = Cookies.get(STORAGE_KEY)
      if (accessToken) {
  ...
  const signIn = useCallback(
    async (data, code) => {
      // console.log(code);
      const { accessToken,user: userData } = await authApi.signIn(data, code);
      const user = await authApi.me({ accessToken,id:userData.id });

      sessionStorage.setItem(STORAGE_KEY, accessToken);
      Cookies.set(STORAGE_KEY, accessToken)
      Cookies.set("id", userData.id)
      Cookies.set("role",userData.user?.role.name)
